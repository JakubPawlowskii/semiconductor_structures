...
varied = np.linspace(0.0, 1.0, 300)  # value of x
col = []
...
GaAs_layer1 = 500
AlGaAsSb_layers = [100,200,400,500]  # from 100 to 500 nm
GaAs_layer2 = 500
# lb = '(d)'
# --------------------- GaAs prelim --------------------------------------
...
dosq = []
for w, AlGaAsSb_layer in enumerate(AlGaAsSb_layers):
    nq = []
    pq = []
    Efq = []
    n_eq = []
...
ax1.set_xlabel("Ułamek molowy $x$", fontsize=fontsize)
ax2.set_xlabel("Ułamek molowy $x$", fontsize=fontsize)
...
for w in range(len(AlGaAsSb_layers)):
    Ef = np.zeros(len(varied))
    n = np.zeros(len(varied))
    for q in range(len(varied)):
        Ef[q] = Ef_w[w][q]
        n[q] = n_w[w][q]
    ax1.plot(varied, Ef)
    ax2.semilogy(varied, n * 1e21,label='$'+str(AlGaAsSb_layers[w]) + '\;nm$')
